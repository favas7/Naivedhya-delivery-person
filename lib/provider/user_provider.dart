...
class UserProvider with ChangeNotifier {
  ...
  /// Get user profile by user ID with statistics
  Future<bool> getUserProfile(String userId) async {
    try {
      ...
      if (response != null) {
        ...
        await _fetchUserStatistics(userId);
  ...
  /// Fetch user statistics from orders table
  Future<void> _fetchUserStatistics(String userId) async {
    try {
      ...
      final deliveredOrders = (deliveredOrdersResponse as List).length;
      ...
      final activeOrdersResponse = await _supabase
          ...
          .select('order_id')
          .eq('delivery_person_id', userId)
          .inFilter('delivery_status', ['Assigned', 'Picked Up', 'In Transit']);
      
      final activeOrders = (activeOrdersResponse as List).length;
  ...
  /// Get user rating
  double get rating => (_userProfile?['rating'] ?? 0.0).toDouble();
  
  /// Get total deliveries
  int get totalDeliveries => (_userProfile?['total_deliveries'] ?? 0);
  
  /// Get total orders from stats
  int get totalOrders => _userStats?['total_orders'] ?? 0;
  
  /// Get delivered orders from stats
  int get deliveredOrders => _userStats?['delivered_orders'] ?? 0;
  
  /// Get active orders from stats
  int get activeOrders => _userStats?['active_orders'] ?? 0;
}